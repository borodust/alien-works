...
(defun (setf view-shadows-enabled-p) (enabled-p view)
  (%filament:set-shadowing-enabled
   '(claw-utils:claw-pointer %filament:view) view
   :bool enabled-p))


(defun (setf view-blend-mode) (mode view)
  (%filament:set-blend-mode
   '(claw-utils:claw-pointer %filament:view) view
   '%filament:view+blend-mode mode))
